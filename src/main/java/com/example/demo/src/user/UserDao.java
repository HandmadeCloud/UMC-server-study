...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into User (email, password, nickname, phoneNum, userDong) VALUES (?,?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getEmail(), postUserReq.getPassword(), postUserReq.getNickname(),postUserReq.getPhoneNum(),postUserReq.getUserDong()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    public int modifyUserPhoneNum(PatchUserReq patchUserReq) {
        ...
        Object[] modifyUserNameParams = new Object[]{patchUserReq.getPhoneNum(), patchUserReq.getUserIdx()}; // 주입될 값들(nickname, userIdx) 순
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select userIdx, password,email,nickname,phoneNum,userDong,mannerTemp from User where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
    ...
    public List<GetUserRes> getUsersByNickname(String nickname) {
        ...
        return this.jdbcTemplate.query(getUsersByNicknameQuery,
                (rs, rowNum) -> new GetUserRes(
                        ...
                        rs.getString("phoneNum"),
                        rs.getString("userDong"),
                        rs.getString("mannerTemp")), // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
